...
**트리 읽는 법** (depth 3, gini 등 군더더기 제거):
> ① `length ≤ 2.5` ? → **예: band** (짧고 넓음 = 같은 위치 여러 센서)
> ② 아니오 → `n_sensors ≤ 2.5` ? → **예: line** (길고 좁음 = 한 센서 길이방향) / **아니오: staircase** (길고 넓음 = 대각 진행)

형태가 단 **2개 측정값(length, n_sensors)으로 완전 분리**됨. 결정적으로 **diagonal·size·edge는 기여 0** — 내가 *대각성(diagonal)* 으로 정의한 staircase가 사실은 *"길고 넓은 결함"* 으로 환원됨. 대각 측정은 length×센서수에 이미 담긴 **중복 정보**였음을 데이터가 폭로함. 화이트박스로 규칙을 압축 시각화했고, CNN은 이 규칙 *너머* 의 raw 공간 패턴(셔플 시 무너지는 정보)을 학습.
